...
namespace Unity.VRTemplate
{
    ...
    public class VideoTimeScrubControl : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (!m_IsDragging && !m_VideoJumpPending)
            {
                if (m_VideoPlayer.frameCount > 0)
                {
                    ...
                    Debug.Log("mslider value is: " + progress);
                
                }

                if((float)m_VideoPlayer.frame/m_VideoPlayer.frameCount > percentToWatch/100)
        ...
        void VideoJump()
        {
            ...
            // we only allow user to jump back, not jump forward
            if (frame < m_LastFrameBeforeScrub)
                m_VideoPlayer.frame = (long)frame;
            else
                m_VideoPlayer.frame = m_LastFrameBeforeScrub;
        }
